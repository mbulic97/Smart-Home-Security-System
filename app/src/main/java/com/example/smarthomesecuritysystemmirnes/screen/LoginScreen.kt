...
fun LoginScreen(modifier: Modifier = Modifier,navController: NavController,authViewModel: AuthViewModel= viewModel()){
    ...
    Column (
        ...
        //Arduino
        Spacer(modifier = Modifier.height(5.dp))
        OutlinedTextField(value = id_arduino, onValueChange = {
            id_arduino = it
        },
            label = {
        ...
        Button(onClick = {
            isLoading = true
            authViewModel.login(email,password,id_arduino){success, errorMessage ->
                if (success){
                    isLoading = false
            ...
            enabled = !isLoading && id_arduino.isNotEmpty() && email.isNotEmpty() && password.isNotEmpty(),
            modifier = Modifier.fillMaxWidth()
                .height(60.dp)) {
            if(isLoading){
